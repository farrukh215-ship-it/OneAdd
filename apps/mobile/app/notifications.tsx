...
export default function NotificationsScreen() {
  ...
  const { notifications, unread, unreadCount, isLoading, isRefetching, refetch, markRead, markAllRead } =
    useNotifications();
  const { preferences, isLoading: preferencesLoading, isUpdating, update } = useNotificationPreferences();
  ...
  return (
    ...
    >
      ...
      <View className="mt-4 rounded-2xl bg-white p-4 shadow-sm">
        <Text className="text-[16px] font-extrabold text-ink">Delivery Preferences</Text>
        <Text className="mt-1 text-xs leading-5 text-ink2">
          Contacts, saved ads aur discovery pushes ko server side se control karo.
        </Text>
        {preferencesLoading || !preferences ? (
          <Text className="mt-3 text-sm text-ink2">Preferences load ho rahi hain...</Text>
        ) : (
          <View className="mt-4 gap-3">
            {[
              { key: 'pushEnabled', label: 'Push Notifications', value: preferences.pushEnabled },
              { key: 'contacts', label: 'Contact Alerts', value: preferences.contacts },
              { key: 'savedUpdates', label: 'Saved Ad Updates', value: preferences.savedUpdates },
              { key: 'newListings', label: 'New Listing Alerts', value: preferences.newListings },
            ].map((item) => (
              <Pressable
                key={item.key}
                onPress={() => void update({ [item.key]: !item.value })}
                className={`flex-row items-center justify-between rounded-2xl border px-4 py-3 ${
                  item.value ? 'border-red/20 bg-[#FFF5F4]' : 'border-border bg-[#F8FAFC]'
                }`}
              >
                <View>
                  <Text className="text-sm font-bold text-ink">{item.label}</Text>
                  <Text className="mt-1 text-xs text-ink2">{item.value ? 'Enabled' : 'Disabled'}</Text>
                </View>
                <View className={`rounded-full px-3 py-2 ${item.value ? 'bg-red' : 'bg-[#E5E7EB]'}`}>
                  <Text className={`text-[10px] font-bold ${item.value ? 'text-white' : 'text-ink2'}`}>
                    {isUpdating ? 'Saving...' : item.value ? 'On' : 'Off'}
                  </Text>
                </View>
              </Pressable>
            ))}
