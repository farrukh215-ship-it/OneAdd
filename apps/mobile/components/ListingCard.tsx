      ...
      {({ pressed }) => (
        <View style={{ opacity: pressed ? 0.9 : 1 }}>
          <View className="relative bg-border" style={{ width: cardWidth, aspectRatio: 1 }}>
            ...
            <View className={`absolute bottom-2 left-2 rounded-md px-2 py-0.5 ${statusMeta.badgeClassName}`}>
              <Text className={`text-[9px] font-bold ${statusMeta.textClassName}`}>{statusMeta.label}</Text>
            </View>
            <View className="absolute right-2 top-2 h-7 w-7 items-center justify-center rounded-full bg-white/90">
              <Text className="text-sm text-ink2">♡</Text>
            </View>
          </View>
